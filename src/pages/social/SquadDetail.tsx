...
export default function SquadDetail() {
  ...
  const bestRank = Math.min(...squad.members.map((m: any) => stats[m.userId]?.rank ?? Number.POSITIVE_INFINITY));
      ...
      <div className="space-y-5">
        ...
        <section className="rounded-2xl border border-border/50 bg-card p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5">
            <StatPill label="Members" value={`${squad.members.length}/${squad.maxMembers ?? 8}`} accent />
        ...
        {isMember ? (
          ...
          <section className="rounded-2xl border border-dashed border-border/60 bg-card p-8 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1 max-w-md mx-auto">
              The squad room, roster ranks and tournament planning unlock once a captain approves you.
            </p>
            {requestStatus === "pending" ? (
              <p className="mt-4 text-xs text-primary font-semibold">Your join request is awaiting approval.</p>
            ) : (
