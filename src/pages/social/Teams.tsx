...
import { CreateSquadDialog, SquadCrest, StatPill, gameLabel, initials } from "@/components/squads/squad-ui";
...
export default function Teams() {
  ...
  const discover = useMemo(() => {
    ...
    return allSquads
      ...
      .filter((s: any) => (q ? `${s.name} ${s.tag} ${gameLabel(s.game)}`.toLowerCase().includes(q) : true));
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1 max-w-lg">
                Create a clan, invite players, chat in a members-only squad room, schedule tournaments and see
...
function SquadCard({ squad, member, action }: { squad: any; member?: boolean; action?: React.ReactNode }) {
  const body = (
    <>
