...
export function CreateSquadDialog({ trigger }: { trigger?: React.ReactNode }) {
  ...
  const submit = async () => {
    ...
    setSaving(true);
    try {
      const id = await api.createSquad({ name, tag, game, bio, color, isPublic, ownerId: me.userId });
      await refresh();
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-[1fr_110px] gap-3">
            <div className="space-y-1.5">
              ...
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Midnight Mavericks" maxLength={40} />
            </div>
            <div className="space-y-1.5">
          ...
          <div className="flex items-center justify-between rounded-xl border border-border/50 px-3 py-2.5">
            ...
            <Switch checked={isPublic} onCheckedChange={setIsPublic} aria-label="Discoverable squad" />
          </div>
        </div>
        <DialogFooter>
            ...
            {term.trim().length < 2 ? (
              ...
              results.map((p: any) => (
                <div key={p.user_id} className="flex items-center gap-3 p-3">
                  ...
                  <Button size="sm" variant="outline" disabled={busy === p.user_id} onClick={() => invite(p)}>
                    {busy === p.user_id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Invite"}
                  </Button>
                </div>
              ))
            )}
